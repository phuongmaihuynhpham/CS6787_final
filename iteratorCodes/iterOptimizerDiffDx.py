...
def TRDFO(prob, p, delta_0, d_x=15, total_iter_num=50, h_k=10 ** (-10), eta_k=1):
    result_list = []
    x_k = x_init
    delta_k = delta_0
...
def exper_h_eta(prob, p, delta_0, d_x, h, eta=1):
    result_list = TRDFO(prob, p, delta_0, d_x=d_x, h_k=h, eta_k=eta)
    k_hist = []
    f_k_hist = []
    b_k_hist = []
...
for j in range(len(alpha_space)):
    ...
    for d_x in dx:
        ...
        for i in range(len(h_space)):
            f_k_hist, b_k_hist = exper_h_eta(prob, p, delta_0, d_x, h_space[i])
...
plt.title(r"Reward values (sum $f_k$) for different $\alpha$, varying $d_x$")
...
plt.title(r"Reward values (last $f_k$) for different $\alpha$, varying $d_x$")
plt.xlabel(r"Parameter $h = h_k$ for all $k$")
plt.ylabel("reward value")
plt.legend(loc="best")
plt.show()
